...
async def MergeSub(filePath: str, subPath: str, user_id):
    ...
    maps += f"-map 1:s "
    metadata += f"-metadata:s:s:{subTrack} title='Track {subTrack+1} - tg@PokeTide' "
    subTrack += 1
    LOGGER.info("Sub muxing")
    subprocess.call(
        f"ffmpeg -hide_banner {input_files}-map 0:v:0 -map 0:a:? -map 0:s:? {maps}{metadata}-c:v copy -c:a copy -c:s srt './downloads/{str(user_id)}/[@PokeTide]_softmuxed_video.mkv' ",
        shell=True,
    )
    orgFilePath = shutil.move(
        f"./downloads/{str(user_id)}/[@PokeTide]_softmuxed_video.mkv", filePath
    )
    return orgFilePath
...
def MergeSubNew(filePath: str, subPath: str, user_id, file_list):
    ...
    for j in range(1, (len(file_list))):
        maps += f"-map {j}:s "
        metadata += (
            f"-metadata:s:s:{subTrack} title='Track {subTrack+1} - tg@PokeTide' "
        )
        subTrack += 1
    LOGGER.info("Sub muxing")
    subprocess.call(
    ...
    return f"./downloads/{str(user_id)}/[@PokeTide]_softmuxed_video.mkv"
...
def MergeAudio(videoPath:str,files_list:list,user_id):
    ...
    subprocess.call(
        f"ffmpeg -hide_banner {inputfiles}-map 0:v:0 -map 0:a:? {maps}-map 0:s:? -c:v copy -c:a copy -c:s copy {rmDispositions}-disposition:a:{audioTracks} default './downloads/{str(user_id)}/[@PokeTide]_export.mkv'",
        shell=True
    )
    return f"./downloads/{str(user_id)}/[@PokeTide]_export.mkv"
